...
class add_from_server {
	...
	function admin_init() {
		...
		//Register our JS & CSS
		wp_register_style ('add-from-server', '/wp-content/themes/ultra/library/plugins/add-from-server/add-from-server.css', array(), $this->version);
	...
	function handle_imports() {
		...
		if ( !empty($_POST['files']) && !empty($_POST['cwd']) ) {
			...
			$gallery_year = isset($_REQUEST['gallery-year']) ? $_REQUEST['gallery-year'] : null;
			...
			foreach ( (array)$files as $file ) {
				...
				if ($wp_filetype['type'] == 'application/zip') {
					$zip = new ZipArchive;
					if ($zip->open($filename) === true) {
						for($i = 0; $i < $zip->numFiles; $i++) {
							$curr_filename = $zip->getNameIndex($i);
							$curr_fileinfo = pathinfo($curr_filename);
							$ext = strtolower($curr_fileinfo['extension']);
							if ($ext == 'jpg' || $ext == 'jpeg') {
								$zip->extractTo($this->folder . '/tmp/', array($curr_filename));
								$id = $this->handle_import_file($this->folder . '/tmp/' . $curr_filename, $post_id, $import_date, $gallery_year);
								$this->after_import_file($curr_filename, $id, $import_to_gallery);
								unlink($this->folder . '/tmp/' . $curr_filename);
							}
						}
					} else {
						echo '<div class="updated error"><p>' . sprintf(__('<em>%s</em> was <strong>not</strong> imported due to an error', 'add-from-server'), esc_html($file) ) . '</p></div>';
					}
				} else {
					$id = $this->handle_import_file($filename, $post_id, $import_date, $gallery_year);
					$this->after_import_file($file, $id, $import_to_gallery);
				}
			}
		}
	}

	function after_import_file ($file, $id, $import_to_gallery) {
		if ( is_wp_error($id) ) {
	...
	function handle_import_file($file, $post_id = 0, $import_date = 'file', $gallery_year = null) {
		set_time_limit(120);
		...
		if ( !is_wp_error($id) ) {
			...
			if ($gallery_year) {
				wp_set_post_terms($id, $gallery_year, 'gallery_year');
			}
		}
	...
	function main_content() {
		...
		<?php
			...
			foreach ( (array)$files as $key => $file ) {
				if ( ! $unfiltered_upload ) {
					...
					if ('image/jpeg' !== $wp_filetype['type'] && 'application/zip' !== $wp_filetype['type']) {
		...
		<fieldset>
			<label for="gallery-year">Gallery to add to:</label>
			<select name="gallery-year">
			<?php
			foreach(ultra_get_gallery_years() as $gallery_year) {
			?>
				<option value="<?php echo $gallery_year->term_id; ?>"><?php echo $gallery_year->name; ?></option>
			<?php
			}
			?>
			</select>

			<input type="hidden" name="import-date" value="current" />
		</fieldset>
		<br class="clear" />
